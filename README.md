...
- Starts Vite + `openwork-server` with stable tokens forced into the UI — tokens are reused across relaunches, so open tabs never turn into 401s (`--rotate-tokens` forces fresh ones)
