...
export function AutomationEditor(props: AutomationEditorProps) {
  ...
  useEffect(() => {
    setInput(props.initial ?? defaultInput(props.modelOptions))
  }, [props.initial, props.modelOptions])
    ...
    >
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={(event) => setInput((current) => ({ ...current, name: event.currentTarget.value }))}
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          onChange={(event) => setInput((current) => ({ ...current, instructions: event.currentTarget.value }))}
        />
        <p className="text-xs text-muted-foreground">Each run starts a fresh Cloud thread with these instructions.</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-muted/30 p-3 text-sm text-muted-foreground">
        This Automation runs remotely on Den using your selected model and the OpenWork Connect integrations available to your account. It does not have access to your computer or local files.
      </div>
