...
import { automationCloudThreadRoute, automationExecutionIdentity } from "./automation-cloud-thread"
...
export function AutomationsPage() {
  ...
  if (selectedId) {
    ...
    const threadMatches = !selectedThreadId || selectedReceipt?.run.cloudThread?.id === selectedThreadId
      ...
      <div className="mx-auto max-w-5xl space-y-5 p-6">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]">
          <div className="space-y-5">
            ...
            <Card variant="outline">
              <CardHeader>
                <CardTitle>Remote execution</CardTitle>
                <CardDescription>This Automation runs in Den, independent of this computer.</CardDescription>
              </CardHeader>
                  ...
                  >
                      ...
                      <span className="flex flex-wrap items-center gap-2">
                        ...
                        {run.cloudThread ? (
                          <span className="flex items-center gap-1 text-xs text-muted-foreground">
                            <Cloud className="size-3" />{automationExecutionIdentity(run.cloudThread).label}
                          </span>
                        ) : null}
                      </span>
                    ...
                    <span className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        onClick={() => {
                          if (run.cloudThread) navigate(automationCloudThreadRoute(run.cloudThread))
          ...
          <Card variant="outline" className="h-fit">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Cloud className="size-4" />Cloud thread</CardTitle>
              <CardDescription>{selectedRunId ? "Run receipt and event timeline" : "Select a run to inspect its Cloud thread."}</CardDescription>
            </CardHeader>
            <CardContent>
              {!selectedRunId ? (
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant={runVariant(selectedReceipt.run.status)}>{selectedReceipt.run.status}</Badge>
                    {selectedReceipt.run.cloudThread ? (
                      <Badge variant="outline"><Cloud />{automationExecutionIdentity(selectedReceipt.run.cloudThread).label}</Badge>
                    ) : <Badge variant="outline">Preparing Cloud thread</Badge>}
                  </div>
    ...
    <div className="mx-auto max-w-5xl space-y-5 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Repeatable work that runs remotely in Den using OpenWork Connect.</p>
        </div>
