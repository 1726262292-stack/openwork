...
- Flue's model-facing `mcp__<server>__<tool>` names are projected to OpenCode's `<server>_<tool>` wire IDs. Thus `mcp__openwork-cloud__search_capabilities` is served as `openwork-cloud_search_capabilities`, matching `OPENWORK_CLOUD_EXPECTED_TOOLS`. IDs, descriptions, and parameter schemas come only from tools returned by the live MCP connection; failed, disabled, and unsupported servers contribute no tools.
- Effective global + workspace runtime MCP config is checked at facade initialization, before every facade request, and before harness initialization. Config hashes detect runtime-DB changes without a process restart. A changed or explicitly reconnected entry establishes a replacement connection, closes the prior connection, and invalidates the harness so the next prompt gets the new tool array. Disconnect and dispose close clients; per-server failures are retained as safe status codes and cannot prevent healthy peers, provider listing, or prompting.
