...
function localKey(value: Pick<LocalDateTime, "year" | "month" | "day" | "hour" | "minute">): number {
...
function resolveLocalOccurrence(
  ...
  // A minute scan is intentionally simple and deterministic. It handles zones
  // with non-hour offsets, repeated fall-back minutes, and missing spring-forward
  // wall times without depending on a process-global timezone.
  for (let candidate = start; candidate <= end; candidate += 60_000) {
    ...
    if (key > targetKey && (shifted === null || candidate < shifted)) shifted = candidate;
  }
  return shifted === null ? null : { timestamp: shifted, shifted: true };
}

function isScheduledDay(schedule: ScheduledTaskSchedule, weekday: number): boolean {
...
export function scheduledTaskOccurrences(
  ...
  // Five weekly occurrences need at most 35 days. The larger bound also
  // comfortably covers unusual timezone transitions and malformed calendars.
...
export function scheduledTaskOccurrenceId(input: {
  taskId: string;
  ...
  return `occ_${input.taskId}_${input.taskRevisionId}_${input.trigger}_${occurrence}`;
}
