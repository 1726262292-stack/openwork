# Deploy OpenWork EE on Linux with the native tarball

Status: self-host operator guide (community-supported, best effort)
Related: `scripts/den/selfhost`, `packaging/helm/openwork-ee`, [GitHub issue #2084](https://github.com/different-ai/openwork/issues/2084)

This path installs Den natively on one Linux host with systemd, without Docker
or Kubernetes. It is ideal for a Proxmox LXC or a small VM. This is a
community-supported, best-effort path. The [Helm chart](../packaging/helm/openwork-ee/README.md)
remains the recommended production path. See the cloud guides for
[AWS](aws-eks-helm.md), [Azure](azure-aks-helm.md), and [Google Cloud](gcp-gke-helm.md).
...
Kubernetes operators should use the [Helm chart](../packaging/helm/openwork-ee/README.md).
...
preserves that file on later installs. Keep it readable only by the service
user. The web origin values must match the public URL served by the reverse
proxy.
