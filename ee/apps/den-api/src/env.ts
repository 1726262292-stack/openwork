...
export const env = {
  ...
  automations: {
    enabled: (parsed.DEN_AUTOMATIONS_ENABLED ?? "1").trim() === "1",
    pollIntervalMs: Number(parsed.DEN_AUTOMATIONS_POLL_INTERVAL_MS ?? "15000"),
    batchSize: Number(parsed.DEN_AUTOMATIONS_BATCH_SIZE ?? "25"),
    maxConcurrency: Number(parsed.DEN_AUTOMATIONS_MAX_CONCURRENCY ?? "4"),
    leaseMs: Number(parsed.DEN_AUTOMATIONS_LEASE_MS ?? "60000"),
    runTimeoutMs: Number(parsed.DEN_AUTOMATIONS_RUN_TIMEOUT_MS ?? "900000"),
  },
