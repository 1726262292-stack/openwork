...
export const AGENT_MCP_INSTRUCTIONS = [
  ...
  "For a custom React Artifact, create or update the saved Script with an explicit JSON Schema outputSchema matching its returned data before calling save_artifact_view. React is injected into view source, so use React APIs without imports and render only the supplied data prop. A failed view build includes diagnostics: change the source once and retry with the returned artifactViewId; do not search for a different Artifact tool or call render_dynamic_artifact. After a successful save, call the exact render_artifact_* or preview_artifact_* tool named in its result.",
