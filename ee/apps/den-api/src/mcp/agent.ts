...
import { executeMarketplaceCapability, listAccessibleMarketplaceSkillDescriptors, normalizeRemoteSkillDescription, parseMarketplaceCapabilityName, searchMarketplaceCapabilities, type MarketplaceCapabilityObjectType, type RemoteSkillDescriptor } from "./marketplace-capabilities.js"
...
export function buildAgentSkillIndex(skills: RemoteSkillDescriptor[]): OpenWorkAgentSkillIndex {
  return openworkAgentSkillIndexSchema.parse({
    ...
    skills: skills.map((skill) => openworkAgentSkillIndexEntrySchema.parse({
      name: skill.name,
