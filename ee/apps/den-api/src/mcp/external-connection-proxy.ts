...
export function createExternalConnectionProxyServer(input: {
  ...
  const server = new McpServer(input.descriptor.serverInfo ?? {
    ...
    instructions: input.appHostClient
      ...
      : "Provider MCP surfaces are not exposed to model or legacy clients. Use search_capabilities and execute_capability on the central OpenWork Cloud MCP server.",
  })
...
/**
 * Exposes one member-authorized connection only to Desktop's private App host.
 * Model-facing and legacy clients receive an empty protocol-valid surface;
 * ordinary provider capabilities stay on the central search/execute gateway.
 */
