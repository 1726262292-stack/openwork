 ...
 */
export const ORGANIZATION_CAPABILITY_KEYS = ["installLinks", "mcpConnections", "cloud", "orgProviderSync"] as const
...
export function normalizeOrganizationCapabilities(metadata: MetadataInput): OrganizationCapabilities {
  ...
  return {
    ...
    orgProviderSync: raw.orgProviderSync === true,
  }
}
...
export function readOrganizationCapabilityOverrides(metadata: MetadataInput): Partial<OrganizationCapabilities> {
  ...
  if (typeof raw.orgProviderSync === "boolean") {
    capabilities.orgProviderSync = raw.orgProviderSync
  }
