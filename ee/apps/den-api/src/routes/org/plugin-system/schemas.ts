...
export const libraryItemSchema = z.discriminatedUnion("type", [
  z.object({
    ...
    sourceRepositoryUrl: z.string().trim().min(1).max(1024).nullable(),
    remoteMcpAppId: configObjectIdSchema.nullable(),
    remoteMcpAppStatus: z.enum(["active", "retired"]).nullable(),
    remoteMcpAppActiveVersionId: configObjectVersionIdSchema.nullable(),
    edges: z.array(effectiveAccessEdgeSchema),
    role: accessRoleSchema,
  }),
  z.object({
