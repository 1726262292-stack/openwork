...
beforeAll(async () => {
  seedRequiredEnv()
  ;({ resolveAutomationModelAccessWithStore } = await import("../src/automations/authority.js"))
})
...
const openWorkProvider: AutomationAuthorityProvider = {
  ...
  providerConfig: { api: "https://models.example.test/api/v1" },
  apiKey: "server-side-openwork-key",
}
const customProvider: AutomationAuthorityProvider = {
  id: createDenTypeId("llmProvider"),
  source: "custom",
  name: "Team Provider",
  providerConfig: { api: "https://models.example.test/v1" },
  apiKey: "server-side-custom-key",
}
  ...
  test("resolves enabled OpenWork aliases through the owner's managed provider", async () => {
    ...
    expect(result).toMatchObject({
      ...
      value: {
        ...
        providerId: "openwork",
        modelId: "z-ai/glm-5.2",
        apiKey: openWorkProvider.apiKey,
      },
    })
