...
import { ArrowLeft, CalendarClock, Cloud, Play } from "lucide-react";
  ...
  if (!automationId) {
    ...
    return (
      <div className="mx-auto max-w-[980px] px-6 py-8 md:px-8">
        <div><h1 className="text-[20px] font-semibold tracking-[-0.02em] text-gray-950">Automations</h1><p className="mt-1 text-[13px] text-gray-400">Manage exact-version saved Scripts and their durable OpenWork Cloud history.</p></div>
        <div className="mt-6 grid gap-3 md:grid-cols-2">{items.length === 0 ? <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center text-[13px] text-gray-400">No Automations yet. Create one from a saved Script in Library.</div> : items.map((item) => <button key={item.automation.id} type="button" onClick={() => setQuery(item.automation.id)} className="rounded-2xl border border-gray-100 bg-white p-4 text-left transition hover:border-gray-200 hover:bg-gray-50"><div className="flex items-start justify-between gap-3"><div><h2 className="text-[14px] font-semibold text-gray-900">{item.automation.name}</h2><p className="mt-1 text-[12px] text-gray-400">{item.revision.action?.kind === "saved_script" ? `Exact Script version ${item.revision.action.script.configObjectVersionId.slice(0, 8)}` : item.revision.instructions}</p></div><span className={`rounded-full px-2 py-0.5 text-[10px] ${statusTone(item.automation.state)}`}>{item.automation.state.replace("_", " ")}</span></div><p className="mt-4 text-[11px] text-gray-400">{item.latestRun ? `Last run ${item.latestRun.status}` : "Not run yet"}</p></button>)}</div>
      </div>
    );
  }
