...
ALTER TABLE `config_object` MODIFY COLUMN `object_type` enum('skill','agent','command','tool','mcp','hook','context','custom','workflow','app') NOT NULL;--> statement-breakpoint
ALTER TABLE `connector_mapping` MODIFY COLUMN `object_type` enum('skill','agent','command','tool','mcp','hook','context','custom','workflow','app') NOT NULL;--> statement-breakpoint
