...
// Narration is loaded from the approved script (evals/voiceovers/engine-attested-apply.md).
// The runner fails this flow if the narration drifts from that script.
const vo = await loadVoiceoverParagraphs("engine-attested-apply");

export default {
  id: "engine-attested-apply",
  ...
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("TODO: claim for frame 1", {
          voiceover: vo[0],
          // "I'm on a corporate laptop behind a TLS-inspecting proxy; my agent's cloud co"
          action: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("TODO: claim for frame 2", {
          voiceover: vo[1],
          // "After the update, OpenWork applies its managed config by writing one file, r"
          action: async () => {
            // TODO: drive the app as the end user (ctx.clickText, ctx.fill, ...)
          },
          assert: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("TODO: claim for frame 3", {
          voiceover: vo[2],
          // "The diagnostics now show \"applied — confirmed by engine\", with the desired a"
          action: async () => {
            // TODO: drive the app as the end user (ctx.clickText, ctx.fill, ...)
          },
          assert: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("TODO: claim for frame 4", {
          voiceover: vo[3],
          // "The old network probe still runs as a secondary check, but when the corporat"
          action: async () => {
            // TODO: drive the app as the end user (ctx.clickText, ctx.fill, ...)
          },
          assert: async () => {
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("TODO: claim for frame 5", {
          voiceover: vo[4],
          // "I copy the sanitized diagnostic for support, and for the first time it tells"
          action: async () => {
            // TODO: drive the app as the end user (ctx.clickText, ctx.fill, ...)
          },
          assert: async () => {
