...
test("the welcome screen folds the server-URL door into the join-organization field", async ({ evidence }) => {
  ...
  expect(dialogSource).toContain("saveControlPlaneUrl(parsed.origin)");
  expect(dialogSource).toContain("platform.openLink(parsed.url)");

  evidence.fact(
    "The welcome screen has one paste field for invite links, install links, server URLs, and sign-in codes",
    "The separate Using OpenWork on-premises affordance is gone from Welcome; the join dialog classifies install link, then invite link, then server URL, then sign-in code, pointing the app at the pasted organization before opening web invites.",
    true,
  );
});
