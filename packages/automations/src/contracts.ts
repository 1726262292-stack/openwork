import type { AutomationRevision, AutomationRunTrigger } from "@openwork/types/automations"
...
export function automationOccurrenceIdentity(input: AutomationOccurrenceIdentityInput): {
  ...
  const occurrence = input.scheduledFor === null ? input.nonce : String(input.scheduledFor)
  if (!occurrence) throw new Error("A manual occurrence needs a nonce")
  const stable = [input.automationId, input.revisionId, input.trigger, occurrence]
