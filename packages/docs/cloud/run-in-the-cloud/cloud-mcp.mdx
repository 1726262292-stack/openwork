...
```text
https://api.app.openworklabs.com/mcp/agent
```
...
```json
{
  "mcp": {
    "openwork": {
      "type": "remote",
      "enabled": true,
      "url": "https://api.app.openworklabs.com/mcp/agent",
      "oauth": {}
    }
  }
}
...
```sh
codex mcp add openwork --url https://api.app.openworklabs.com/mcp/agent
...
- The protected resource is exactly `https://api.app.openworklabs.com/mcp/agent`.
...
- OAuth authorize and token requests must include exactly one `resource` value: `https://api.app.openworklabs.com/mcp/agent`.
...
- Public OAuth access tokens are JWTs signed and validated with EdDSA. The issuer is exactly `https://app.openworklabs.com/api/auth`, the audience is exactly `https://api.app.openworklabs.com/mcp/agent`, and access tokens expire after 45 minutes.
...
- **401 missing or invalid token**: run the client's auth/login command again. Confirm the client points to `https://api.app.openworklabs.com/mcp/agent`.
