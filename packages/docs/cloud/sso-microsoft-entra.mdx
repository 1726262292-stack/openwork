...
| Sign on URL | The OpenWork **Sign-in URL** shown after saving SAML, for example `https://app.openworklabs.com/sso/<org-slug>` |
...
OpenWork organization SAML is SP-initiated. Start the test from the OpenWork
Sign-in URL, not from the Entra My Apps tile or an Entra IdP-initiated launch.
...
save. Copy the OpenWork **Sign-in URL** into Entra's **Sign on URL**.
...
| `/?error=unsolicited_response&error_description=IdP-initiated+SSO+not+allowed` | The flow started from Entra as IdP-initiated SAML | Open the OpenWork **Sign-in URL** directly, or set Entra **Sign on URL** to that OpenWork URL and launch from there |
