...
3. **Every EE release becomes MIT after one year.** The EE License contains
   an irrevocable grant: each version of the `ee/` code converts to MIT on
   the first anniversary of its public release. This is a floor, not a
   marketing line — it is written into the license itself. If OpenWork ever
   disappeared, the newest release would be fully open source within a year.
...
- **Requires an Enterprise plan (white-labeling):** replacing OpenWork
